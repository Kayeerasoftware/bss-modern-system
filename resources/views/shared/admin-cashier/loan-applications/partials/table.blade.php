        ...
        <tr class="transition-all duration-200 {{ $loop->iteration % 2 == 0 ? 'bg-yellow-50' : 'bg-white' }} hover:bg-gradient-to-r hover:from-orange-50 hover:to-red-50 border-l-4 border-yellow-500">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-center">
                <div class="flex items-center justify-center gap-2">
                    <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loan-applications.show', $app->id) }}" class="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition-all duration-200 group" title="View">
                        <i class="fas fa-eye group-hover:scale-110 transition-transform"></i>
                    </a>
                    <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loan-applications.edit', $app->id) }}" class="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200 transition-all duration-200 group" title="Edit">
                        <i class="fas fa-edit group-hover:scale-110 transition-transform"></i>
                    </a>
                    <form action="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loan-applications.destroy', $app->id) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
