...
class LoanSettingsController extends Controller
{
    public function index()
    {
        if (!Schema::hasTable('loan_settings')) {
            return redirect()->route('cashier.settings.index')->with('error', 'Loan settings table is missing. Please run migrations.');
        }

        $settings = LoanSetting::first() ?? new LoanSetting();
        return view('cashier.loan-settings', ['settings' => $settings->toArray()]);
    }
    ...
    public function update(Request $request)
    {
        ...
        return redirect()->route('cashier.loan-settings')->with('success', 'Loan settings updated successfully!');
    }
}
