<nav class="topnav no-print fixed top-0 left-0 right-0 h-12 bg-gradient-to-r from-violet-200 via-blue-200 to-cyan-200 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 backdrop-blur-xl border-b border-violet-300 dark:border-gray-600 z-50 shadow-2xl ring-2 ring-gradient-to-r ring-from-violet-400 ring-to-cyan-400 ring-opacity-80 shadow-violet-500/30 dark:shadow-gray-900/30 transition-colors duration-300">
    <div class="nav-container flex items-center justify-between h-full px-6 max-w-full mx-auto">
        <div class="nav-left flex items-center gap-2">
            ...
            <div class="logo flex items-center gap-2 font-bold text-sm tracking-tight px-3 py-1.5 bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl border border-blue-200 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md hover:border-blue-300 cursor-pointer" @click="showLogoModal = !showLogoModal" :class="showLogoModal ? 'ring-2 ring-blue-400' : ''">
                <img src="{{ asset('assets/images/logo.png') }}" alt="BSS Logo" class="logo-img h-6 w-auto object-contain drop-shadow-md transition-all duration-300">
        ...
        <div class="nav-center flex-1 flex justify-center">
            <div class="flex items-center gap-2 font-bold text-sm tracking-tight px-6 py-1 rounded-xl border transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md cursor-pointer hidden lg:flex" style="background: linear-gradient(135deg, #fae8ff, #f3e8ff); border-color: #a855f7;" @click="showShareholderModal = !showShareholderModal" :class="showShareholderModal ? 'ring-2 ring-purple-400' : ''">
                <div class="bg-white/20 p-0.5 rounded-lg backdrop-blur-sm">
                    <i class="fas fa-chart-pie text-sm drop-shadow-md" style="color: #9333ea;"></i>
                </div>
                <span class="text-sm font-bold whitespace-nowrap" style="background: linear-gradient(135deg, #9333ea, #a855f7); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">Shareholder Dashboard</span>
            </div>
        ...
        <div class="nav-right flex items-center gap-2">
            <div class="relative" x-data="{
                ...
                <button @click="showCalendar = !showCalendar" class="nav-btn relative flex items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 border border-blue-200 px-3 py-1.5 rounded-xl cursor-pointer transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md hover:border-blue-300" title="Calendar" :class="showCalendar ? 'ring-2 ring-blue-400' : ''">
                    ...
                    <span class="bg-gradient-to-br from-blue-600 to-purple-600 bg-clip-text text-transparent text-xs font-bold font-mono ml-2 hidden md:inline" x-text="currentTime.toLocaleDateString() + ' ' + currentTime.toLocaleTimeString()" x-cloak></span>
                </button>
                <div x-show="showCalendar" @click.away="showCalendar = false" x-transition x-cloak class="fixed sm:absolute top-16 sm:top-full left-1/2 -translate-x-1/2 sm:left-auto sm:right-0 sm:translate-x-0 mt-0 sm:mt-3 w-[320px] bg-white rounded-xl shadow-2xl z-[100] overflow-hidden border-2 border-blue-100">
                    <div class="px-4 py-2.5 bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-between">
                    ...
                    <div class="p-3">
                        ...
                        <div class="grid grid-cols-7 gap-1">
                            ...
                            <template x-for="day in getDaysInMonth(currentMonth, currentYear)" :key="day">
                                <div class="h-9 flex items-center justify-center text-xs rounded-lg cursor-pointer transition" :class="isToday(day) ? 'bg-gradient-to-br from-purple-500 to-pink-500 text-white font-bold shadow-md' : 'hover:bg-purple-50 text-gray-700'" x-text="day"></div>
                            </template>
                        </div>
                    </div>
                </div>
            </div>
            <a href="{{ route('shareholder.notifications.index') }}" @click="activeLink = 'notifications'" class="nav-btn relative flex items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 border border-blue-200 px-3 py-1.5 rounded-xl cursor-pointer transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md hover:border-blue-300" title="Notifications">
            ...
            <button @click="showProfileDropdown = !showProfileDropdown" class="flex items-center gap-2 px-3 py-1.5 rounded-xl cursor-pointer transition-all duration-300 bg-gradient-to-br from-blue-50 to-purple-50 border border-blue-200 hover:-translate-y-0.5 hover:shadow-md hover:border-blue-300">
                <span class="bg-gradient-to-br from-blue-600 to-purple-600 bg-clip-text text-transparent text-xs font-bold tracking-tight hidden lg:inline" x-text="'Welcome back, ' + adminProfile.name">Welcome back, Shareholder</span>
...
<!-- Profile Dropdown (Outside Nav) -->
<div x-show="showProfileDropdown" @click.away="showProfileDropdown = false" x-transition class="fixed top-14 right-6 w-44 bg-white rounded-xl shadow-2xl z-40 border border-blue-100 overflow-hidden" style="display: none;">
    <!-- Arrow -->
    <div class="absolute -top-2 right-6 w-4 h-4 bg-gradient-to-br from-purple-500 to-pink-500 border-l border-t border-blue-100 transform rotate-45"></div>
    
    <!-- Header with gradient background -->
    <div class="pt-6 pb-3 px-3 text-center bg-gradient-to-br from-purple-500 to-pink-600 relative">
        ...
        <div class="relative">
            ...
            <p class="text-sm font-bold text-white truncate" x-text="adminProfile.name">Shareholder</p>
            <p class="text-xs text-purple-100" x-text="adminProfile.role">Shareholder</p>
        </div>
    </div>
    
    <!-- Menu -->
    <div class="py-1.5">
        <a href="{{ route('shareholder.profile') }}" @click="showProfileDropdown = false" class="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-purple-50 transition group">
        ...
        <a href="#" @click="showProfileDropdown = false; activeLink = 'settings'" class="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-pink-50 transition group">
            <div class="w-7 h-7 rounded-lg bg-pink-100 flex items-center justify-center group-hover:bg-pink-200 transition">
                <i class="fas fa-cog text-pink-600 text-xs"></i>
            </div>
        ...
        <div class="flex items-center justify-between px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition">
            <div class="flex items-center gap-2">
                <div class="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center transition">
                    <i class="fas fa-moon text-gray-600 text-xs" x-show="!$store.darkMode.on"></i>
                    <i class="fas fa-sun text-yellow-500 text-xs" x-show="$store.darkMode.on"></i>
                </div>
                <span class="font-medium text-xs" x-text="$store.darkMode.on ? 'Light Mode' : 'Dark Mode'">Dark Mode</span>
            </div>
            <button @click="$store.darkMode.toggle()" class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors" :class="$store.darkMode.on ? 'bg-purple-600' : 'bg-gray-300'">
                <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform" :class="$store.darkMode.on ? 'translate-x-5' : 'translate-x-0.5'"></span>
            </button>
        </div>
    </div>
...
<div x-show="showLogoModal" x-transition @click.away="showLogoModal = false" x-cloak class="fixed top-12 left-0 right-0 bg-white border-b border-gray-200 shadow-xl z-[100] p-6">
    <div class="max-w-md mx-auto text-center">
        <img src="{{ asset('assets/images/logo.png') }}" alt="BSS Logo" class="h-16 w-auto object-contain mx-auto mb-3 drop-shadow-lg">
...
<div x-show="showShareholderModal" x-transition @click.away="showShareholderModal = false" x-cloak class="fixed top-12 left-0 right-0 bg-white border-b border-gray-200 shadow-xl z-[100] p-6">
    <div class="max-w-md mx-auto text-center">
        <div class="w-16 h-16 mx-auto mb-3 bg-gradient-to-br from-purple-400 to-pink-500 rounded-full flex items-center justify-center">
            <i class="fas fa-chart-pie text-white text-2xl"></i>
        </div>
        <h2 class="text-lg font-bold bg-gradient-to-br from-purple-600 to-pink-600 bg-clip-text text-transparent mb-2">Shareholder Dashboard</h2>
        <p class="text-sm text-gray-600 mb-1">Investment portfolio & dividend management</p>
        <p class="text-xs text-gray-400">Track your investments and returns</p>
    </div>
</div>

@include('partials.admin.modals.chat')
