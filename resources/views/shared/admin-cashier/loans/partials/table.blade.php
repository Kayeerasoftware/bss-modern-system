<table class="min-w-full divide-y-0">
    ...
    <tbody class="bg-white">
        ...
        <tr class="transition-all duration-200 {{ $loop->iteration % 2 == 0 ? 'bg-green-50' : 'bg-white' }} hover:bg-gradient-to-r hover:from-teal-50 hover:to-blue-50 border-l-4 border-green-500">
            ...
            <td class="px-6 py-4 whitespace-nowrap border-r border-gray-200">
                @php
                    $interestAmount = (float) ($loan->interest_amount ?? 0);
                    $totalInterest = (float) ($loan->total_interest ?? 0);
                    $displayInterest = $interestAmount > 0 ? $interestAmount : $totalInterest;
                @endphp
                <div class="space-y-1">
                    <div class="flex items-center gap-2">
                        <i class="fas fa-percentage text-teal-500"></i>
                        <span class="text-xs text-gray-500">Rate</span>
                        <span class="text-sm font-semibold text-gray-900">{{ number_format((float) ($loan->interest_rate ?? 0), 2) }}%</span>
                    </div>
                    <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-semibold text-gray-900">{{ number_format($displayInterest, 2) }}</span>
                    </div>
                </div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap border-r border-gray-200">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-center">
                <div class="flex items-center justify-center gap-2">
                    <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loans.show', $loan->id) }}" class="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition-all duration-200 group" title="View">
                        <i class="fas fa-eye group-hover:scale-110 transition-transform"></i>
                    </a>
                    <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loans.edit', $loan->id) }}" class="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200 transition-all duration-200 group" title="Edit">
                        <i class="fas fa-edit group-hover:scale-110 transition-transform"></i>
                    </a>
                    <form action="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loans.destroy', $loan->id) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
