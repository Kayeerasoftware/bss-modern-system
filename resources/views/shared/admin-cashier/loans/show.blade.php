<div class="min-h-screen bg-gradient-to-br from-green-50 via-teal-50 to-blue-50 p-3 md:p-6">
    ...
    <div class="flex items-center justify-between gap-3 mb-6">
        <div class="flex items-center gap-3">
            <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loans.index') }}" class="p-3 bg-white rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:scale-105">
        ...
        <div class="flex gap-2">
            <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loans.print', $loan->id) }}" class="px-4 py-2 bg-gradient-to-r from-red-600 to-pink-600 text-white rounded-xl hover:shadow-xl transition-all font-bold transform hover:scale-105">
                <i class="fas fa-file-pdf mr-2"></i>Print PDF
            </a>
            <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loans.edit', $loan->id) }}" class="px-4 py-2 bg-gradient-to-r from-green-600 to-teal-600 text-white rounded-xl hover:shadow-xl transition-all font-bold transform hover:scale-105">
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                ...
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row gap-4">
                        <form action="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loans.approve', $loan->id) }}" method="POST" class="flex-1">
                            @csrf
                        ...
                        <form action="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loans.reject', $loan->id) }}" method="POST" class="flex-1">
                            @csrf
