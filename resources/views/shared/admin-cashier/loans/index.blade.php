<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-3 md:p-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3 mb-3 md:mb-4">
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg p-2 md:p-3 text-white shadow-lg transform hover:scale-105 transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-green-100 text-[8px] md:text-[10px] font-medium mb-0.5">Total Applications</p>
        ...
        <div class="bg-gradient-to-br from-teal-500 to-teal-600 rounded-lg p-2 md:p-3 text-white shadow-lg transform hover:scale-105 transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-base md:text-xl font-bold">{{ number_format($applications->sum('amount')) }}</h3>
                </div>
                <div class="bg-white/20 p-1.5 md:p-2 rounded-lg backdrop-blur-sm">
    ...
    <div class="bg-gradient-to-br from-green-50 via-teal-50 to-blue-50 rounded-2xl shadow-lg border border-green-100 overflow-hidden mb-4">
        <form method="GET" action="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loans.index') }}" x-data="{ showAdvanced: false }">
            <div class="bg-white/60 backdrop-blur-sm p-3">
                ...
                <div class="bg-white/80 rounded-xl p-2.5 border border-green-100">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-2">
                        <div class="md:col-span-4 relative">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search applications..." class="w-full pl-8 pr-2 py-1.5 text-xs border border-green-200 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all bg-white">
                        </div>
                        <div class="md:col-span-2 relative">
                            <i class="fas fa-circle absolute left-2.5 top-1/2 transform -translate-y-1/2 text-teal-400 text-[8px]"></i>
                            <select name="status" class="w-full pl-8 pr-2 py-1.5 text-xs border border-teal-200 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all appearance-none bg-white" @change="$el.form.submit()">
                        ...
                        <div class="md:col-span-2 flex gap-1">
                            <button type="submit" class="flex-1 px-2 py-1.5 bg-gradient-to-r from-green-600 to-teal-600 text-white rounded-lg hover:shadow-lg transition-all text-xs font-bold">
                                <i class="fas fa-search"></i>
                            </button>
                            <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.loans.index') }}" class="flex-1 px-2 py-1.5 bg-gray-500 text-white rounded-lg hover:shadow-lg transition-all text-xs font-bold text-center">
                                <i class="fas fa-redo"></i>
                            </a>
                            <button type="button" @click="showAdvanced = !showAdvanced" class="flex-1 px-2 py-1.5 bg-blue-500 text-white rounded-lg hover:shadow-lg transition-all text-xs font-bold">
                ...
                <!-- Advanced Filters -->
                <div x-show="showAdvanced" x-collapse class="mt-2 bg-white/80 rounded-xl p-2.5 border border-green-100">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                        <div class="relative">
                            <label class="text-[10px] font-bold text-gray-700 mb-1 block">Amount Range</label>
                            <div class="flex gap-1">
                                <input type="number" name="amount_min" value="{{ request('amount_min') }}" placeholder="Min" class="w-1/2 px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500">
                                <input type="number" name="amount_max" value="{{ request('amount_max') }}" placeholder="Max" class="w-1/2 px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500">
                            </div>
                        </div>
                        <div class="relative">
                            <label class="text-[10px] font-bold text-gray-700 mb-1 block">Date From</label>
                            <input type="date" name="date_from" value="{{ request('date_from') }}" class="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500">
                        </div>
                        <div class="relative">
                            <label class="text-[10px] font-bold text-gray-700 mb-1 block">Date To</label>
                            <input type="date" name="date_to" value="{{ request('date_to') }}" class="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <!-- Loans Table -->
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100" id="loans-table">
