...
class DepositController extends Controller
{
    ...
    {
        $query = Transaction::whereHas('transactionType', fn ($q) => $q->where('name', 'deposit'))
        ...
        if (!empty($validated['transaction_category_id'])) {
            ...
            if (!$category || (int) $category->transaction_type_id !== (int) $transactionTypeId) {
                return back()->withErrors([
                    'transaction_category_id' => 'Category must match deposit type.',
                ])->withInput();
            }
            $categoryId = $category->id;
        } else {
            $categoryId = TransactionCategory::query()
                ->where('name', $validated['category'] ?? 'savings_deposit')
    ...
    {
        $transaction = Transaction::whereHas('transactionType', fn ($q) => $q->where('name', 'deposit'))
            ->with(['member', 'transactionType', 'paymentMethod', 'statusRelation'])
            ->findOrFail($id);
