<div class="mb-6">
    ...
    <div class="grid grid-cols-4 gap-4 mb-6">
        @php
            $deposits = $data->filter(function($t) { 
                return optional($t->transactionType)->name === 'deposit' || $t->type === 'deposit';
            });
            $withdrawals = $data->filter(function($t) { 
                return optional($t->transactionType)->name === 'withdrawal' || $t->type === 'withdrawal';
            });
            $totalIncome = $deposits->sum('amount');
            $totalExpenses = $withdrawals->sum('amount');
        @endphp
        <div class="bg-green-50 p-4 rounded-lg">
            <p class="text-sm text-gray-600">Total Income</p>
            <p class="text-2xl font-bold text-green-600">UGX {{ number_format($totalIncome) }}</p>
        </div>
        <div class="bg-red-50 p-4 rounded-lg">
            <p class="text-sm text-gray-600">Total Expenses</p>
            <p class="text-2xl font-bold text-red-600">UGX {{ number_format($totalExpenses) }}</p>
        </div>
        <div class="bg-blue-50 p-4 rounded-lg">
            <p class="text-sm text-gray-600">Net Balance</p>
            <p class="text-2xl font-bold text-blue-600">UGX {{ number_format($totalIncome - $totalExpenses) }}</p>
        </div>
...
<table class="w-full">
    ...
    <tbody>
        ...
        <tr class="border-b">
            ...
            <td class="px-4 py-3">{{ ucfirst($transaction->type ?? optional($transaction->transactionType)->name ?? 'N/A') }}</td>
            <td class="px-4 py-3">{{ optional($transaction->member)->full_name ?? 'N/A' }}</td>
