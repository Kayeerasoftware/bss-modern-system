@php
    ...
    $editRoute = $editRoute ?? route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.financial.transactions.edit', $transaction->id);
...
<div class="min-h-screen bg-gradient-to-br from-cyan-50 via-blue-50 to-indigo-50 p-3 md:p-6">
    ...
    <div class="flex items-center justify-between gap-3 mb-6">
        ...
        <div class="flex gap-2">
            @if($showEdit && $editRoute)
                <a href="{{ $editRoute }}" class="px-4 py-2 bg-gradient-to-r from-yellow-600 to-orange-600 text-white rounded-xl hover:shadow-xl transition-all font-bold transform hover:scale-105">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            @if($transaction->type === 'transfer' && !empty($transaction->metadata) && !empty($transaction->metadata['transfer_to_member_id']))
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-indigo-100">
                ...
                <div class="p-6 space-y-4">
                    <p class="text-sm text-gray-700">
                        <span class="font-semibold">{{ $transaction->member->full_name ?? 'Sender' }}</span>
                        transferred
                        <span class="font-semibold">UGX {{ number_format($transaction->amount, 2) }}</span>
                        to
                        <span class="font-semibold">{{ $transaction->metadata['transfer_to_member_name'] ?? 'Recipient' }}</span>
                        ({{ $transaction->metadata['transfer_to_member_id'] }}).
                    </p>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="rounded-xl border border-blue-100 bg-blue-50 p-4">
                            <p class="text-xs uppercase text-blue-600 font-semibold">From</p>
                            <p class="font-bold text-gray-900">{{ $transaction->member->full_name ?? 'N/A' }}</p>
                            <p class="text-sm text-gray-600">{{ $transaction->member_id }}</p>
                        </div>
                        <div class="rounded-xl border border-emerald-100 bg-emerald-50 p-4">
                            <p class="text-xs uppercase text-emerald-600 font-semibold">To</p>
                            <p class="font-bold text-gray-900">{{ $transaction->metadata['transfer_to_member_name'] ?? 'Member' }}</p>
                            <p class="text-sm text-gray-600">{{ $transaction->metadata['transfer_to_member_id'] }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        @if($transaction->type === 'transfer' && !empty($transaction->metadata) && !empty($transaction->metadata['transfer_to_member_id']))
                        <div class="p-4 bg-gray-50 rounded-xl">
                            <p class="text-xs text-gray-600 mb-1">Transfer From</p>
                            <p class="font-bold text-gray-900">{{ $transaction->member->full_name ?? 'N/A' }} ({{ $transaction->member_id }})</p>
                        </div>
                        <div class="p-4 bg-gray-50 rounded-xl">
                            <p class="text-xs text-gray-600 mb-1">Transfer To</p>
                            <p class="font-bold text-gray-900">
                                {{ $transaction->metadata['transfer_to_member_name'] ?? 'Member' }}
                                ({{ $transaction->metadata['transfer_to_member_id'] }})
                            </p>
                        </div>
                        @endif
