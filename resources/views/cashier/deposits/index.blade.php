...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-3 md:p-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg p-2 md:p-3 text-white shadow-lg">
            ...
            <h3 class="text-xl font-bold">{{ $deposits->total() }}</h3>
        </div>
        <div class="bg-gradient-to-br from-teal-500 to-teal-600 rounded-lg p-2 md:p-3 text-white shadow-lg">
            ...
            <h3 class="text-xl font-bold">{{ number_format($deposits->sum('amount')/1000000, 1) }}M</h3>
        </div>
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg p-2 md:p-3 text-white shadow-lg">
            ...
            <h3 class="text-xl font-bold">{{ $deposits->where('created_at', '>=', today())->count() }}</h3>
        </div>
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg p-2 md:p-3 text-white shadow-lg">
            ...
            <h3 class="text-xl font-bold">{{ $deposits->where('created_at', '>=', now()->startOfMonth())->count() }}</h3>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-br from-green-50 via-teal-50 to-green-50 rounded-2xl shadow-lg border border-green-100 overflow-hidden mb-4">
        <form method="GET" action="{{ route('cashier.deposits.index') }}" x-data="{ showAdvanced: false }">
                ...
                <div class="bg-white/80 rounded-xl p-2.5 border border-green-100">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-2">
                        <div class="md:col-span-5 relative">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search member..." class="w-full pl-8 pr-2 py-1.5 text-xs border border-green-200 rounded-lg focus:ring-2 focus:ring-green-500 bg-white">
                        </div>
                        ...
                        <div class="md:col-span-4 flex gap-1.5">
                            ...
                            <a href="{{ route('cashier.deposits.index') }}" class="px-2 py-1.5 text-xs bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700 rounded-lg hover:shadow-md transition font-semibold">
            ...
            <table class="min-w-full divide-y-0">
                ...
                <tbody class="bg-white">
                    ...
                    <tr class="transition-all duration-200 {{ $loop->iteration % 2 == 0 ? 'bg-green-50' : 'bg-white' }} hover:bg-gradient-to-r hover:from-green-50 hover:to-teal-50 border-l-4 border-green-500">
                        <td class="px-6 py-4 whitespace-nowrap border-r border-gray-200 text-sm">{{ $deposit->created_at->format('M d, Y H:i') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap border-r border-gray-200">
                            <div class="flex items-center gap-3">
                                @if($deposit->member)
                                @if($deposit->member->profile_picture)
                                    <img src="{{ asset('storage/' . $deposit->member->profile_picture) }}" class="w-10 h-10 rounded-full object-cover ring-2 ring-green-500 ring-offset-2">
                                @else
                                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-green-500 to-teal-600 flex items-center justify-center ring-2 ring-green-500 ring-offset-2">
                                        <span class="text-white font-bold">{{ substr($deposit->member->full_name, 0, 1) }}</span>
                                    </div>
                                @endif
                                <div>
                                    <p class="text-sm font-semibold text-gray-900">{{ $deposit->member->full_name }}</p>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap border-r border-gray-200 text-sm font-bold text-green-600">+{{ number_format($deposit->amount) }}</td>
                        <td class="px-6 py-4 text-sm text-gray-500 border-r border-gray-200">{{ $deposit->description ?? '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <a href="{{ route('cashier.deposits.show', $deposit->id) }}" class="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200 transition inline-block">
