...
<div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-3 md:p-6">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            <a href="{{ route('shareholder.notifications.index') }}" class="p-3 bg-white rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:scale-105">
        ...
        <div class="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
                        ...
                        <div class="flex flex-wrap gap-2 justify-center md:justify-start">
                            ...
                            <span class="px-4 py-1.5 bg-white/20 backdrop-blur-sm text-white rounded-full text-sm font-bold">
                                <i class="fas fa-circle text-xs mr-1 
                                    @if($notification->is_read) text-green-400
                                    @else text-yellow-400
                                    @endif"></i>{{ $notification->is_read ? 'Read' : 'Unread' }}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-6 md:p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 border-2 border-blue-100">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-xs text-gray-500">Read Status</p>
                                    <p class="font-bold text-gray-900">{{ $notification->is_read ? 'Read on ' . $notification->updated_at->format('M d, Y') : 'Unread' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border-2 border-purple-100">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <h4 class="text-lg font-bold text-gray-800">Details</h4>
                        </div>
                        <div class="space-y-3">
                            <div class="flex items-center gap-3">
                            ...
                            <div class="flex items-center gap-3">
                                <i class="fas fa-flag text-orange-600 w-5"></i>
                                <div>
                                    <p class="text-xs text-gray-500">Priority</p>
                                    <p class="font-bold text-gray-900">{{ ucfirst($notification->priority ?? 'normal') }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="font-bold text-gray-900">{{ $notification->sender ?? 'BSS System' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex flex-col sm:flex-row justify-end gap-4 pt-6 mt-6 border-t-2 border-gray-100">
                    <a href="{{ route('shareholder.notifications.index') }}" class="px-8 py-3 border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 transition-all font-bold text-center transform hover:scale-105">
