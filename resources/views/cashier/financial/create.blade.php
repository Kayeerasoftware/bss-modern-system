...
<div class="min-h-screen bg-gradient-to-br from-cyan-50 via-blue-50 to-indigo-50 p-3 md:p-6">
    ...
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('cashier.financial.index') }}" class="p-3 bg-white rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:scale-105">
            <i class="fas fa-arrow-left text-cyan-600"></i>
        </a>
        <div>
            <h2 class="text-2xl md:text-3xl font-bold bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-600 bg-clip-text text-transparent">New Transaction</h2>
            <p class="text-gray-600 text-sm">Create a new financial transaction</p>
        </div>
    </div>

    <form action="{{ route('cashier.financial.store') }}" method="POST" class="max-w-6xl mx-auto">
        @csrf
        ...
        <div class="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
            ...
            <div class="bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-600 p-8 text-center">
                ...
                <p class="text-white/80 text-sm">Record a new transaction with complete details</p>
            </div>
            ...
            <div class="p-6 md:p-8 space-y-8">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="space-y-2">
                            ...
                            <select name="member_id" id="member_id" required onchange="loadMemberBalance()" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all text-sm appearance-none bg-white">
                                <option value="">Select Member</option>
                                @foreach($members as $member)
                                    <option value="{{ $member->member_id }}" data-balance="{{ $member->balance ?? 0 }}" data-savings="{{ $member->savings ?? 0 }}">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="type" id="type" required onchange="calculateTransaction()" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-sm appearance-none bg-white">
                        ...
                        <div class="space-y-2 md:col-span-3">
                            ...
                            <textarea name="notes" rows="2" placeholder="Internal notes (not visible to member)..." class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-all text-sm"></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row justify-end gap-4 pt-6 border-t-2 border-gray-100">
                    <a href="{{ route('cashier.financial.index') }}" class="px-8 py-3 border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 transition-all font-bold text-center transform hover:scale-105">
...
function loadMemberBalance() {
    ...
    if (option.value) {
        ...
        totalSavings = parseFloat(option.dataset.savings) || 0;
