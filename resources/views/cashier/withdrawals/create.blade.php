...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-red-50 via-pink-50 to-purple-50 p-3 md:p-6">
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('cashier.withdrawals.index') }}" class="p-3 bg-white rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:scale-105">
            <i class="fas fa-arrow-left text-red-600"></i>
        </a>
        <div class="flex-1">
            <h2 class="text-2xl md:text-3xl font-bold bg-gradient-to-r from-red-600 via-pink-600 to-purple-600 bg-clip-text text-transparent">New Withdrawal</h2>
            <p class="text-gray-600 text-sm">Process a new withdrawal transaction</p>
        </div>
    </div>

    <form action="{{ route('cashier.withdrawals.store') }}" method="POST" class="max-w-4xl mx-auto">
        @csrf

        <div class="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
            <div class="bg-gradient-to-r from-red-600 via-pink-600 to-purple-600 p-8 text-center">
                <div class="flex justify-center mb-4">
                    <div class="w-32 h-32 rounded-full bg-white flex items-center justify-center shadow-2xl">
                        <i class="fas fa-arrow-up text-red-600 text-6xl"></i>
                    </div>
                </div>
                <h3 class="text-white text-xl font-bold">Withdrawal Transaction</h3>
                <p class="text-white/80 text-sm">Fill in the withdrawal details below</p>
            </div>
            ...
            <div class="p-6 md:p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <select name="member_id" id="member_id" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all text-sm appearance-none bg-white" onchange="loadMemberInfo()">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="amount_display" id="amount_display" value="{{ old('amount') }}" required oninput="formatAmount(this);"
                                   class="w-full pl-16 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-pink-500 focus:border-pink-500 transition-all text-sm">
                        </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="category" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all text-sm appearance-none bg-white">
                            ...
                            <option value="savings" {{ old('category') == 'savings' ? 'selected' : '' }}>Savings Withdrawal</option>
                            <option value="shares" {{ old('category') == 'shares' ? 'selected' : '' }}>Shares Withdrawal</option>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <textarea name="description" rows="3" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-pink-500 focus:border-pink-500 transition-all text-sm" placeholder="Add any notes about this withdrawal...">{{ old('description') }}</textarea>
                    </div>
                ...
                <div class="flex flex-col sm:flex-row justify-end gap-4 pt-6 border-t-2 border-gray-100">
                    <a href="{{ route('cashier.withdrawals.index') }}" class="px-8 py-3 border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 transition-all font-bold text-center transform hover:scale-105">
                        <i class="fas fa-times mr-2"></i>Cancel
                    </a>
                    <button type="submit" id="submit_btn" class="px-8 py-3 bg-gradient-to-r from-red-600 via-pink-600 to-purple-600 text-white rounded-xl hover:shadow-2xl transition-all font-bold transform hover:scale-105">
                        <i class="fas fa-check mr-2"></i>Process Withdrawal
                    </button>
                </div>
            </div>
        </div>
...
function calculateNewBalance() {
    ...
    if (amount > 0 && memberSavings >= 0) {
        ...
        if (amount > memberSavings) {
            warningText.textContent = 'Warning: Withdrawal amount exceeds available balance!';
