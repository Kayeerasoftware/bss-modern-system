<div class="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 p-3 md:p-6">
        ...
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg p-2 md:p-3 text-white shadow-lg transform hover:scale-105 transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-[8px] md:text-[10px] text-blue-100">Reconciled Savings: {{ number_format($summary['reconciled_savings'] ?? 0, 0) }}</p>
                </div>
                <div class="bg-white/20 p-1.5 md:p-2 rounded-lg backdrop-blur-sm">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
        @php
            $quickReports = [
                ['type' => 'members', 'name' => 'Members', 'icon' => 'users', 'color' => 'yellow-600 to-amber-600'],
                ['type' => 'financial', 'name' => 'Financial', 'icon' => 'chart-line', 'color' => 'green-600 to-emerald-600'],
                ['type' => 'loans', 'name' => 'Loans', 'icon' => 'hand-holding-usd', 'color' => 'orange-600 to-red-600'],
                ['type' => 'transactions', 'name' => 'Transactions', 'icon' => 'exchange-alt', 'color' => 'cyan-600 to-teal-600'],
                ['type' => 'projects', 'name' => 'Projects', 'icon' => 'project-diagram', 'color' => 'indigo-600 to-purple-600'],
                ['type' => 'audit', 'name' => 'Audit', 'icon' => 'clipboard-list', 'color' => 'red-600 to-pink-600'],
            ];
        @endphp

        @foreach($quickReports as $report)
        <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100 transform hover:scale-105 transition-all">
            <div class="bg-gradient-to-r from-{{ $report['color'] }} p-4 text-center">
                <i class="fas fa-{{ $report['icon'] }} text-white text-2xl mb-2"></i>
                <h3 class="text-white text-sm font-bold">{{ $report['name'] }}</h3>
            </div>
            <div class="p-3">
                <form action="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.reports.generate') }}" method="POST" class="quick-report-form">
                    @csrf
                    <input type="hidden" name="type" value="{{ $report['type'] }}">
                    <div class="mb-2">
                        <label class="text-xs text-gray-600 font-semibold">From</label>
                        <input type="date" name="from_date" value="{{ date('Y-m-01') }}" required class="w-full px-2 py-1 border rounded text-xs">
                    </div>
                    <div class="mb-2">
                        <label class="text-xs text-gray-600 font-semibold">To</label>
                        <input type="date" name="to_date" value="{{ date('Y-m-d') }}" required class="w-full px-2 py-1 border rounded text-xs">
                    </div>
                    <input type="hidden" name="format" value="html">
                    <button type="submit" class="w-full px-3 py-2 bg-gradient-to-r from-{{ $report['color'] }} text-white rounded-lg hover:shadow-lg transition-all text-xs font-bold">
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 mb-6">
        ...
        <div class="p-6">
            @if ($errors->any())
                <div class="mb-4 bg-red-50 border-l-4 border-red-500 p-4 rounded-lg">
                    <div class="flex items-center mb-2">
                        ...
                        <h4 class="text-red-800 font-bold">Validation Errors</h4>
                    </div>
            ...
            <form action="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.reports.generate') }}" method="POST" id="reportForm">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    <div class="space-y-2">
                        ...
                        <select name="type" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all text-sm appearance-none bg-white @error('type') border-red-500 @enderror">
                            <option value="">Select Type</option>
                            <option value="members" {{ old('type') == 'members' ? 'selected' : '' }}>Members Report</option>
                            <option value="financial" {{ old('type') == 'financial' ? 'selected' : '' }}>Financial Report</option>
                            <option value="loans" {{ old('type') == 'loans' ? 'selected' : '' }}>Loans Report</option>
                            <option value="transactions" {{ old('type') == 'transactions' ? 'selected' : '' }}>Transactions Report</option>
                            <option value="projects" {{ old('type') == 'projects' ? 'selected' : '' }}>Projects Report</option>
                            <option value="audit" {{ old('type') == 'audit' ? 'selected' : '' }}>Audit Report</option>
                        </select>
                        @error('type')
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="date" name="from_date" value="{{ old('from_date') }}" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all text-sm @error('from_date') border-red-500 @enderror">
                        @error('from_date')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="space-y-2">
                        ...
                        <input type="date" name="to_date" value="{{ old('to_date') }}" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all text-sm @error('to_date') border-red-500 @enderror">
                        @error('to_date')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="space-y-2">
                        ...
                        <select name="format" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-sm appearance-none bg-white @error('format') border-red-500 @enderror">
                            <option value="html" {{ old('format') == 'html' ? 'selected' : '' }}>HTML/PDF</option>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gradient-to-r hover:from-emerald-50 hover:to-teal-50 transition-all duration-200">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600">
                            <i class="fas fa-calendar text-teal-500 mr-1"></i>{{ $report->generated_at ? $report->generated_at->format('M d, Y H:i') : 'N/A' }}
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-center">
                            <div class="flex items-center justify-center gap-1">
                                <button class="p-2 text-blue-600 hover:bg-blue-100 rounded-lg transition-all transform hover:scale-110" title="Download">
                                    <i class="fas fa-download"></i>
                                </button>
                                <button class="p-2 text-green-600 hover:bg-green-100 rounded-lg transition-all transform hover:scale-110" title="View">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <button class="p-2 text-red-600 hover:bg-red-100 rounded-lg transition-all transform hover:scale-110" title="Delete">
...
<script>
// Form validation for main report form
...
// Form validation for quick report forms
const quickReportForms = document.querySelectorAll('.quick-report-form');
quickReportForms.forEach(form => {
...
new Chart(transactionCtx, {
    type: 'doughnut',
    data: {
        labels: ['Deposits', 'Withdrawals', 'Transfers', 'Loans'],
        datasets: [{
            data: [45, 25, 20, 10],
            backgroundColor: [
                'rgb(16, 185, 129)',
