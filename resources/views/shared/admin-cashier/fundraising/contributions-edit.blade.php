<div class="min-h-screen bg-gradient-to-br from-yellow-50 via-orange-50 to-amber-50 p-3 md:p-6">
    <div class="flex items-center justify-between gap-3 mb-6">
        <div class="flex items-center gap-3">
            <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.fundraising.contributions.show', [$fundraising->id, $contribution->id]) }}" class="p-3 bg-white rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:scale-105">
                <i class="fas fa-arrow-left text-yellow-600"></i>
            </a>
            <div>
                <h2 class="text-2xl md:text-3xl font-bold bg-gradient-to-r from-yellow-600 to-orange-600 bg-clip-text text-transparent">Edit Contribution</h2>
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 max-w-3xl">
        ...
        <form action="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.fundraising.contributions.update', [$fundraising->id, $contribution->id]) }}" method="POST" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="member_id" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">
                        ...
                        @foreach($members as $member)
                            ...
                            @endphp
                            <option value="{{ $member->id }}" @selected(old('member_id', $contribution->member_id) == $member->id)>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="funding_source" id="funding_source" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none" required>
                        <option value="deposit" @selected(old('funding_source', $fundingSource ?? 'deposit') === 'deposit')>Deposit (Cash/Bank/Mobile)</option>
                        <option value="savings_transfer" @selected(old('funding_source', $fundingSource ?? '') === 'savings_transfer')>Transfer From Savings</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="contributor_name" value="{{ old('contributor_name', $contribution->contributor_name) }}" list="contributors-list" autocomplete="off" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none" required>
                ...
                <div>
                    ...
                    <input type="email" name="contributor_email" value="{{ old('contributor_email', $contribution->contributor_email) }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Contributor Phone</label>
                    <input type="text" name="contributor_phone" value="{{ old('contributor_phone', $contribution->contributor_phone) }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Contributor Address</label>
                    <input type="text" name="contributor_address" value="{{ old('contributor_address', $contribution->contributor_address) }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">
                </div>
                <div class="flex items-center gap-2">
                    <input type="hidden" name="is_anonymous" value="0">
                    <input type="checkbox" id="is_anonymous" name="is_anonymous" value="1" class="rounded text-yellow-600" @checked(old('is_anonymous', $contribution->is_anonymous))>
                ...
                <div>
                    ...
                    <input type="number" name="amount" min="100" step="1" value="{{ old('amount', $contribution->amount) }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none" required>
                ...
                <div>
                    ...
                    <input type="date" name="contribution_date" value="{{ old('contribution_date', optional($contribution->contribution_date)->format('Y-m-d')) }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none" required>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Payment Method</label>
                    <select name="payment_method_id" id="payment_method_id" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none" required>
                        <option value="">Select method</option>
                        @foreach($paymentMethods as $method)
                            <option value="{{ $method->id }}" @selected(old('payment_method_id', $contribution->payment_method_id) == $method->id)>{{ $method->display_name ?? $method->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Receipt Number</label>
                    <input type="text" name="receipt_number" value="{{ old('receipt_number', $contribution->receipt_number) }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="flex items-center gap-2">
                    ...
                    <input type="checkbox" id="receipt_issued" name="receipt_issued" value="1" class="rounded text-yellow-600" @checked(old('receipt_issued', $contribution->receipt_issued))>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="receipt_issued_at" value="{{ old('receipt_issued_at', optional($contribution->receipt_issued_at)->format('Y-m-d\\TH:i')) }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Receipt Issued By</label>
                    <select name="receipt_issued_by" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">
                        <option value="">Auto (Current User)</option>
                        @foreach($receiptIssuers as $issuer)
                            <option value="{{ $issuer->id }}" @selected(old('receipt_issued_by', $contribution->receipt_issued_by) == $issuer->id)>{{ $issuer->username ?? $issuer->email }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex items-center gap-2">
                    <input type="hidden" name="thank_you_sent" value="0">
                    <input type="checkbox" id="thank_you_sent" name="thank_you_sent" value="1" class="rounded text-yellow-600" @checked(old('thank_you_sent', $contribution->thank_you_sent))>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="thank_you_sent_at" value="{{ old('thank_you_sent_at', optional($contribution->thank_you_sent_at)->format('Y-m-d\\TH:i')) }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">
                </div>
            </div>

            <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">Public Message</label>
                <textarea name="message" rows="3" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">{{ old('message', $contribution->message) }}</textarea>
                <div class="flex items-center gap-2 mt-2">
                    <input type="hidden" name="is_public_message" value="0">
                    <input type="checkbox" id="is_public_message" name="is_public_message" value="1" class="rounded text-yellow-600" @checked(old('is_public_message', $contribution->is_public_message))>
            ...
            <div>
                ...
                <textarea name="notes" rows="4" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">{{ old('notes', $contribution->notes) }}</textarea>
            </div>

            <div class="flex gap-2">
                <button id="contribution_submit" type="submit" class="px-5 py-2 bg-gradient-to-r from-yellow-600 to-orange-600 text-white rounded-lg font-bold hover:shadow-lg transition-all">
                    <i class="fas fa-save mr-2"></i>Update Contribution
                </button>
                <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.fundraising.contributions.show', [$fundraising->id, $contribution->id]) }}" class="px-5 py-2 bg-white text-gray-700 rounded-lg font-bold border border-gray-200 hover:shadow-lg transition-all">
                    Cancel
                </a>
            </div>
        </form>
