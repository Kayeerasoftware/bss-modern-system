<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-3 md:p-6">
    ...
    <div class="bg-white/90 rounded-2xl shadow-lg border border-cyan-100 p-3 mb-4">
        <div class="flex flex-wrap gap-2">
            <a href="{{ request()->fullUrlWithQuery(['category' => null, 'page' => 1]) }}"
               class="px-3 py-1.5 text-xs font-semibold rounded-full border {{ request('category') ? 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50' : 'bg-gradient-to-r from-cyan-600 to-blue-600 text-white border-transparent' }}">
                All Categories
                <span class="ml-1 text-[10px] font-bold">{{ $transactions->total() }}</span>
            </a>
            @foreach($categories as $category)
                ...
                <a href="{{ request()->fullUrlWithQuery(['category' => $category->name, 'page' => 1]) }}"
                   class="px-3 py-1.5 text-xs font-semibold rounded-full border {{ $isActive ? 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white border-transparent' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50' }}">
    ...
    <div class="bg-gradient-to-br from-cyan-50 via-blue-50 to-indigo-50 rounded-2xl shadow-lg border border-cyan-100 overflow-hidden mb-4">
        <form method="GET" action="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.financial.transactions') }}">
                ...
                <div class="bg-white/80 rounded-xl p-2.5 border border-cyan-100">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-2">
                        ...
                        <div class="md:col-span-3 flex gap-1.5">
                            ...
                            <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.financial.transactions') }}" class="px-2 py-1.5 text-xs bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700 rounded-lg hover:shadow-md transition-all duration-300 font-semibold">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-cyan-50 via-blue-50 to-indigo-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider" style="min-width: 200px;">Member</th>
                    ...
                    <tr class="hover:bg-gradient-to-r hover:from-cyan-50 hover:to-blue-50 transition-all duration-200">
                        ...
                        <td class="px-4 py-3" style="min-width: 200px;">
                            <div class="flex items-center gap-2">
                                @if($transaction->member && $transaction->member->profile_picture_url)
                                    <img src="{{ $transaction->member->profile_picture_url }}" class="w-10 h-10 rounded-full object-cover ring-2 ring-cyan-500 ring-offset-2" alt="">
                                @else
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-center">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.financial.transactions.show', $transaction->id) }}" class="p-2 text-blue-600 hover:bg-blue-100 rounded-lg transition-all transform hover:scale-110" title="View">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.financial.transactions.edit', $transaction->id) }}" class="p-2 text-yellow-600 hover:bg-yellow-100 rounded-lg transition-all transform hover:scale-110" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route((request()->routeIs('cashier.*') ? 'cashier' : 'admin') . '.financial.transactions.destroy', $transaction->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this transaction?');">
                                    @csrf
                                    @method('DELETE')
